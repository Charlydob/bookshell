...
Categories are read from the canonical finance catalog at
`finance/finance/catalog/categories`. Names are matched ignoring case,
accents, and extra spaces. A category created by the movement endpoint appears
here after the movement is committed.
...
If `categoryId` or `category` names an existing category, the backend reuses it.
If it is a new non-empty expense/income category, the backend creates it in
`finance/finance/catalog/categories` and uses it immediately. Transfers always
use the internal `transfer` category and never create categories from the
request body.

After a movement is committed, the backend sends one Web Push through the
existing Bookshell Web Push subscription infrastructure. Push delivery failure
does not roll back the movement. Replays with the same `Idempotency-Key` return
the stored response and do not send another push.

Push payloads:

- Expense title: `💸 Nuevo gasto`
- Income title: `💰 Nuevo ingreso`
- Transfer title: `🔁 Nueva transferencia`
- Expense/income body: `<original amount> <currency> · <category> · <note>`
- Transfer body: `<original amount> <currency> · <from account> → <to account> · <note>`

Empty fields are omitted without orphan separators.
...
```json
{
  ...
  "placeTypes": ["city", "village"],
  "localCategories": [
    { "key": "cafeteria", "name": "Cafetería", "emoji": "☕" }
  ]
}
```

`placeTypes` is derived from classifications already present in
`world/geography`. `localCategories` comes from Mundo's canonical local-category
structure, `world/categoryEmojis`, plus categories already present in
`world/places`. Existing names and emojis are preserved exactly.

### Create World Place

`POST /shortcuts/world/places`
...
`rating` is optional and is the same 0-10 decimal score used by Mundo's star UI.
It is stored only in the existing `rating` field. The textual `note` field is
still accepted for compatibility if sent, but Apple Shortcuts should not use it
for the score.

For `local`, `category` is matched against Mundo categories ignoring case,
accents, and extra spaces. If it does not exist, a new `world/categoryEmojis`
entry is created and reused by the web UI.
...
```json
{
  ...
  "itemId": "uuid",
  "type": "saved",
  "worldPath": "world/saved/uuid",
  "openUrl": "https://bookshell.charlydob.com/#view-world",
  "item": {}
}
```
...
- the backend derives the user from the Bearer token and ignores client user IDs

Errors include `INVALID_SHORTCUT_TOKEN`, `INVALID_WORLD_TYPE`,
`INVALID_LATITUDE`, `INVALID_LONGITUDE`, `INVALID_RATING`, `INVALID_CAPTURED_AT`,
`DATA_NOT_FOUND`, and `IDEMPOTENCY_CONFLICT`.

Apple Shortcuts conceptual flow:

1. Get Current Location.
2. Extract latitude, longitude, country, region/state, city/locality, and
   address when available.
3. Ask for save type: quick save, place, or local.
4. Ask only for the extra fields needed by that type, including optional
   `rating` for place/local.
5. POST JSON to `/shortcuts/world/places` with the Bearer token and an
   `Idempotency-Key`, for example a UUID generated by Shortcuts.
