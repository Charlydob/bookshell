...
function createEmptyProductsList(seed = {}, { temporary = false } = {}) {
  ...
  const list = normalizeProductsHubList(
    temporary ? '__draft__' : String(seed?.id || createFinanceRecordId('list')).trim(),
    {
      ...
      ticketCategoryId: normalizeProductText(seed?.ticketCategoryId || state.productsHub?.settings?.ticketCategoryId || ''),
...
function renderProductsTicketCategorySelect(model = null) {
  const activeList = model?.activeList || resolveActiveProductsList();
  const categories = categoriesMetaList();
  const current = String(
    model?.activeTicket?.ticketCategoryId
    || activeList?.tickets?.[activeList.activeTicketId || activeList.primaryTicketId || 'ticket-1']?.ticketCategoryId
    || activeList?.ticketCategoryId
    || state.productsHub?.settings?.ticketCategoryId
    || ''
  ).trim();
  const hasCurrent = !current || categories.some((row) => row.name === current);
  const options = [
...
function resolveProductsConverterDraft(model) {
  const ticketId = String(model?.activeTicketId || '').trim();
  const ticketCurrency = String(model?.activeTicket?.ticketCurrency || getDefaultCurrency()).toUpperCase();
  const rateFromEUR = ticketCurrency === 'EUR' ? 1 : Math.max(0.0000001, Number(model?.activeTicket?.exchangeRateFromEUR || (1 / Math.max(0.0000001, Number(model?.activeTicket?.exchangeRateToEUR || 1))) || 1));
  const cached = state.productsReceiptConverterTemp?.[ticketId];
  if (!cached || cached.currency !== ticketCurrency) return { eur: 1, foreign: rateFromEUR, rateFromEUR };
  return {
    eur: Math.max(0.0000001, Number(cached.eur || 1)),
    foreign: Math.max(0.0000001, Number(cached.foreign || rateFromEUR)),
    rateFromEUR,
  };
}
...
function renderProductsTicketHero(model, options = {}) {
  ...
  const receiptError = normalizeProductText(state.productsReceiptError || '');
  return `
      ...
      <div class="productsWorkbench__receipt" data-products-receipt>
        <header>
          ...
          ${ticketCurrency !== 'EUR' ? `
            <div class="productsWorkbench__receiptMetaHint">
              ...
              <span>€ ≈</span>
              <input class="food-control productsWorkbench__receiptMetaControl productsWorkbench__receiptMetaControl--fx" type="number" step="0.01" value="${escapeHtml(Number(converterDraft.foreign || converterDraft.rateFromEUR).toFixed(2))}" data-products-converter-foreign aria-label="${escapeHtml(ticketCurrency)}" />
        ...
        <footer>
          ...
          <div class="productsWorkbench__receiptTotalRow">
            <span>Total</span>
            <strong data-products-ticket-total-footer>${fmtCurrencyCode(receiptActualTotal, ticketCurrency)}</strong>
          </div>
          <div class="productsWorkbench__receiptTotalRow productsWorkbench__receiptTotalRow--muted">
            <span>Previsto</span>
            <strong data-products-list-total>${fmtCurrencyCode(receiptEstimatedTotal, ticketCurrency)}</strong>
          </div>
          ...
          ${ticketCurrency !== 'EUR' ? `<div class="productsWorkbench__receiptTotalRow productsWorkbench__receiptTotalRow--muted"><span>Total en euros</span><strong>≈ ${fmtCurrency(receiptTotalEUR)}</strong></div>` : ''}
          ${renderProductsAccountSelect(model)}
          ${renderProductsTicketCategorySelect(model)}
        </footer>
      </div>
      ${receiptError ? `<p class="productsWorkbench__emptyMini is-negative" data-products-ticket-error role="alert">${escapeHtml(receiptError)}</p>` : ''}
...
function setProductsReceiptBusy(isBusy) {
  ...
  if (!patchProductsTicketHero(buildCurrentProductsModel())) {
    if (state.activeView === 'products') patchProductsShoppingPanel(buildCurrentProductsModel());
    else triggerRender();
  }
}

function setProductsReceiptError(message = '') {
  const nextError = normalizeProductText(message || '');
  if (state.productsReceiptError === nextError) return;
  state.productsReceiptError = nextError;
  if (!patchProductsTicketHero(buildCurrentProductsModel())) {
    if (state.activeView === 'products') patchProductsShoppingPanel(buildCurrentProductsModel());
    else triggerRender();
  }
...
async function confirmProductsTicketFromDom() {
  ...
  try {
    ...
    if (!draft || !draftLineCount) {
      financeReceiptLog('validation:error', { reason: 'empty-lines' });
      const message = 'No hay líneas para confirmar';
      setProductsReceiptError(message);
      toast(message);
      return;
    }
    if (activeTicketMeta.accountedTxId) {
      ...
      const message = 'Este ticket ya está contabilizado';
      setProductsReceiptError(message);
      toast(message);
      return;
    }
    ...
    if (persistedTicketMeta.accountedTxId) {
      ...
      const message = 'Este ticket ya fue contabilizado';
      setProductsReceiptError(message);
      toast(message);
      return;
    }

    const persistedActiveTicketLines = Object.fromEntries(
    ...
    if (!validation.ok) {
      ...
      const message = validation.message || 'No se puede confirmar el ticket';
      setProductsReceiptError(message);
      toast(message);
      return;
    }
    ticketPayload.ticketCategoryId = validation.ticketCategoryId;
    console.log('[ticket:confirm:ticketPayload]', ticketPayload);
    ...
    const estimatedTotal = Object.values(persistedActiveTicketLines || {}).reduce((sum, line) => sum + (Math.max(1, Number(line.qty || 1)) * normalizeProductPositiveNumber(line.estimatedPrice, 0)), 0);
    const remainingLines = Object.fromEntries(
      Object.entries(persistedList.lines || {}).filter(([, line]) => String(line?.ticketId || persistedList.primaryTicketId || '').trim() !== activeTicketId),
    );
    const nextTicketsMeta = Object.fromEntries(
      Object.entries(persistedList.tickets || {})
        .filter(([ticketMetaId]) => ticketMetaId !== activeTicketId)
        .map(([ticketMetaId, ticketMeta]) => [ticketMetaId, normalizeProductsListTicketMeta(ticketMetaId, ticketMeta)]),
    );
    const remainingTicketIds = Object.keys(nextTicketsMeta);
    ...
    const nextPrimaryTicketId = remainingTicketIds.includes(String(persistedList.primaryTicketId || ''))
      ? String(persistedList.primaryTicketId || '')
      : (nextActiveTicketId || remainingTicketIds[0] || String(persistedList.primaryTicketId || activeTicketId || 'ticket-1'));
    const convertedList = ensureProductsListTickets({
      ...
      primaryTicketId: nextPrimaryTicketId,
      activeTicketId: nextActiveTicketId || nextPrimaryTicketId,
    ...
    const visibleMessage = resolveProductsTicketConfirmErrorMessage(error);
    setProductsReceiptError(visibleMessage);
    toast(visibleMessage);
  } finally {
...
view.addEventListener('focusout', async (event) => {
  ...
  view.addEventListener('change', async (event) => {
    ...
    if (event.target.matches('[data-products-converter-reset]')) {
      const formEl = view.querySelector('[data-products-list-form]');
      const ticketId = String(formEl?.dataset.productsActiveTicketId || resolveActiveProductsList()?.activeTicketId || '').trim();
      const currency = String(view.querySelector('[data-products-receipt-currency]')?.value || 'EUR').toUpperCase();
      const rateFromEUR = Math.max(0.0000001, Number(resolveActiveProductsList()?.tickets?.[ticketId]?.exchangeRateFromEUR || 1));
      state.productsReceiptConverterTemp = { ...(state.productsReceiptConverterTemp || {}), [ticketId]: { currency, eur: 1, foreign: rateFromEUR } };
      const eurInput = view.querySelector('[data-products-converter-eur]');
  ...
  view.addEventListener('input', async (event) => {
    ...
    if (event.target.matches('[data-products-converter-eur], [data-products-converter-foreign]')) {
      ...
      if (event.target.matches('[data-products-converter-eur]')) {
        eurValue = Math.max(0.0000001, Number(event.target.value || 1));
        foreignValue = eurValue * rateFromEUR;
        if (foreignInput) foreignInput.value = Number(foreignValue).toFixed(2);
        console.info('[finance:currency] converter-temp-eur', { currency, eur: eurValue, foreign: foreignValue });
      } else {
        foreignValue = Math.max(0.0000001, Number(event.target.value || rateFromEUR));
        eurValue = foreignValue * (1 / rateFromEUR);
        if (eurInput) eurInput.value = Number(eurValue).toFixed(2);
        console.info('[finance:currency] converter-temp-foreign', { currency, eur: eurValue, foreign: foreignValue });
      }
      state.productsReceiptConverterTemp = { ...(state.productsReceiptConverterTemp || {}), [ticketId]: { currency, eur: eurValue, foreign: foreignValue } };
      return;
    }
