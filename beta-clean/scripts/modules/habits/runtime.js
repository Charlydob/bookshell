...
import { logFirebaseBytesRisk, logFirebaseRead, registerViewListener, trackFirebasePayload, trackedGet, trackedOnValue } from "../../shared/firebase/read-debug.js";

let currentUid = auth.currentUser?.uid ?? null;
let BASE = null;
let HABITS_ROOT = null;
let HABITS_PATH = null;
let HABIT_ACTIVE_SESSIONS_PATH = null;
...
let HABITS_SCHEDULE_PATH = null;
let HABITS_SCHEDULE_DAY_CREDITS_PATH = null;
let HABIT_UI_PATH = null;
...
function logHabitsSession(event, detail = {}) {
  try {
    console.info(`[habits:session] ${event}`, {
      uid: currentUid || "",
      at: new Date().toISOString(),
      ...detail,
    });
...
function logHabitsCache(event, detail = {}) {
  try {
...
function estimateHabitsCacheBytes(value) {
  try {
...
let habitWorkSchedules = {}; // { weekStart: { weekStart, rows, resolvedDays, updatedAt } }
let habitQuickPanel = "sessions";
let activeTab = "today";
let runningSession = null; // Sesión principal (derivada de activeSessions)
let activeSessions = {};
let selectedSessionId = null;
let sessionInterval = null;
let sessionStopGuardUntil = 0;
let sessionStopGuardReason = "";

function armSessionStopGuard(reason = "stop-flow", ttlMs = 1200) {
  sessionStopGuardReason = String(reason || "stop-flow").trim() || "stop-flow";
  sessionStopGuardUntil = Date.now() + Math.max(250, Number(ttlMs) || 1200);
}

function isSessionStopGuardActive() {
  return Date.now() < Number(sessionStopGuardUntil || 0);
}
...
let habitSessionLiveUnsubscribers = [];
let habitSessionLiveSlices = new Map();
let habitSessionSyncVersion = 0;
let habitSessionSyncMode = "none";
let habitSessionSyncKey = "";
...
function normalizeWorkScheduleWeek(raw = {}, key = "") {
...
function normalizeWorkSchedulesStore(raw = {}) {
  const out = {};
  if (!raw || typeof raw !== "object") return out;
  Object.entries(raw).forEach(([weekStart, weekData]) => {
    if (!weekStart) return;
    out[weekStart] = normalizeWorkScheduleWeek(weekData || {}, weekStart);
  });
  return out;
}
...
function formatDuration(valueMinutes, mode = "auto") {
  const safeMin = Math.max(0, Math.round(Number(valueMinutes) || 0));
  ...
  if (mode === "min") return `${safeMin}m`;
  if (mode === "h") return mins ? `${hours}h ${mins}m` : `${hours}h`;
  if (mode === "d") {
    if (!days) return mins ? `${hours}h ${mins}m` : `${hours}h`;
    return restHours ? `${days}d ${restHours}h` : `${days}d`;
  }

  if (safeMin < 60) return `${safeMin}m`;
  if (safeMin < 24 * 60) return mins ? `${hours}h ${mins}m` : `${hours}h`;
  return restHours ? `${days}d ${restHours}h` : `${days}d`;
}
...
function resolveHabitGoalForRange(habit, range, start, end) {
  const goals = normalizeGoalsByRange(habit?.goalsByRange || habit || {});
  if (goals[range]) return goals[range];
  const dayGoal = goals.day || (Number(habit?.targetMinutes) > 0 ? Math.round(Number(habit.targetMinutes)) : null);
  if (!dayGoal) return null;
  if (range === "day") return dayGoal;
  if (range === "week") return dayGoal * 7;
  const daysInRange = daysBetweenInclusive(start, end);
  if (range === "month" || range === "year") return dayGoal * Math.max(1, daysInRange);
  if (range === "total") return dayGoal * Math.max(1, daysInRange);
  return null;
}

function formatMinutes(min) {
  return formatDuration(min, habitDurationMode);
}
...
function sanitizeWorkScheduleTimeTyping(value = "") {
  ...
  return `${hours}${hours.length >= 2 || minutes ? ":" : ""}${minutes}`;
}

function ensureWorkScheduleTimeInputValue(input) {
  if (!input) return DEFAULT_TIME_INPUT_VALUE;
  const nextValue = sanitizeWorkScheduleTimeTyping(input.value) || DEFAULT_TIME_INPUT_VALUE;
  if (input.value !== nextValue) input.value = nextValue;
  return nextValue;
}

function selectWorkScheduleTimeSegment(input, segment = "hours") {
  if (!input?.setSelectionRange) return;
  const nextValue = ensureWorkScheduleTimeInputValue(input);
  if (!nextValue.includes(":")) input.value = normalizeWorkScheduleTimeValue(nextValue, DEFAULT_TIME_INPUT_VALUE);
  const isMinutes = segment === "minutes";
  input.setSelectionRange(isMinutes ? 3 : 0, isMinutes ? 5 : 2);
}

function getWorkScheduleTimeSegmentFromCaret(input, fallbackClientX = null) {
  if (!input) return "hours";
  const caret = Number.isInteger(input.selectionStart) ? input.selectionStart : 0;
  if (caret <= 2) return "hours";
  if (caret >= 3) return "minutes";
  if (typeof fallbackClientX === "number") {
    const rect = input.getBoundingClientRect();
    const relativeX = fallbackClientX - rect.left;
    return relativeX > rect.width / 2 ? "minutes" : "hours";
  }
  return "hours";
}
...
function buildResolvedWorkDays(weekStart, rows = []) {
  ...
  rows.forEach((row) => {
    ...
    row.days.forEach((dayKey) => {
      ...
      if (dayIdx < 0) return;
      const dateKey = dateKeyLocal(addDays(startDate, dayIdx));
      const existing = resolved[dateKey] || {
        start: row.start,
        end: row.end,
        shift,
        hours: 0,
...
function getWorkScheduleForWeek(weekStart) {
  return normalizeWorkScheduleWeek(habitWorkSchedules?.[weekStart] || { weekStart }, weekStart);
}

function getResolvedWorkScheduleForDate(dateKey) {
  const date = parseDateKey(dateKey);
  if (!date) return null;
  const weekStart = dateKeyLocal(startOfWeek(date));
  return habitWorkSchedules?.[weekStart]?.resolvedDays?.[dateKey] || null;
}
...
function applyHabitsCachePayload(raw = {}, { persistNormalized = false } = {}) {
  ...
  habitWorkSchedules = normalizeWorkSchedulesStore(parsed.habitWorkSchedules || {});
...
function schedulePersistHabitOfflineSnapshot() {
  ...
  habitSnapshotPersistTimer = window.setTimeout(() => {
    ...
    const payload = buildHabitsCachePayload();
    logHabitsCache("save", {
      bytes: estimateHabitsCacheBytes(payload),
    });
...
async function hydrateHabitsFromOfflineSnapshot() {
  ...
  try {
    ...
    const bytes = estimateHabitsCacheBytes(snapshot.data);
    applyHabitsCachePayload(snapshot.data);
    logHabitsCache("hydrate", {
      bytes,
      ageMs,
    });
    return true;
...
function normalizeHabitModel(raw) {
  ...
  const goalsByRange = normalizeGoalsByRange(raw.goalsByRange || raw);
  return {
    ...raw,
...
const $habitQuickSessionsTab = document.getElementById("habit-quick-tab-sessions");
const $habitQuickCountersTab = document.getElementById("habit-quick-tab-counters");
const $habitQuickWorkSchedulesTab = document.getElementById("habit-quick-tab-work-schedules");
...
const $quickCountersWrap = document.getElementById("quick-counters-wrap");
...
function loadHabitQuickPanel() {
  try {
    const saved = (localStorage.getItem(HABIT_QUICK_PANEL_STORAGE) || "").trim();
    return saved === "counters" ? "counters" : "sessions";
  } catch (_) {
    return "sessions";
  }
}

function persistHabitQuickPanel() {
  try {
    localStorage.setItem(HABIT_QUICK_PANEL_STORAGE, habitQuickPanel === "counters" ? "counters" : "sessions");
  } catch (_) {
    // noop
  }
}

function syncHabitQuickPanelUI() {
  const activePanel = habitQuickPanel === "counters" ? "counters" : "sessions";
  const showCounters = activePanel === "counters";

  $habitQuickSessionsTab?.classList.toggle("is-active", !showCounters);
  $habitQuickSessionsTab?.setAttribute("aria-selected", String(!showCounters));
  $habitQuickCountersTab?.classList.toggle("is-active", showCounters);
  $habitQuickCountersTab?.setAttribute("aria-selected", String(showCounters));

  if ($habitQuickSessionsWrap) {
    $habitQuickSessionsWrap.classList.toggle("is-hidden", showCounters);
    $habitQuickSessionsWrap.open = true;
    $habitQuickSessionsWrap.setAttribute("aria-hidden", String(showCounters));
  }

  if ($quickCountersWrap) {
    $quickCountersWrap.classList.toggle("is-hidden", !showCounters);
    $quickCountersWrap.setAttribute("aria-hidden", String(!showCounters));
  }
}

function setHabitQuickPanel(panel = "sessions", { persist = true } = {}) {
  habitQuickPanel = panel === "counters" ? "counters" : "sessions";
  if (persist) persistHabitQuickPanel();
  syncHabitQuickPanelUI();
}
...
const $habitWorkScheduleModal = document.getElementById("habit-work-schedule-modal");
const $habitWorkScheduleClose = document.getElementById("habit-work-schedule-close");
const $habitWorkScheduleCancel = document.getElementById("habit-work-schedule-cancel");
const $habitWorkScheduleWeek = document.getElementById("habit-work-schedule-week");
const $habitWorkScheduleRows = document.getElementById("habit-work-schedule-rows");
...
function getTodayDetailKey(detail) {
  if (!detail) return "";
  return detail.id || detail.dataset.groupId || "";
}

function findTodayDetailByKey(panel, key) {
  if (!panel || !key) return null;
  const safeId = (window.CSS && CSS.escape) ? CSS.escape(key) : key;
  const byId = panel.querySelector(`#${safeId}`);
  if (byId) return byId;
  return Array.from(panel.querySelectorAll('details[data-group-id]')).find((detail) => detail.dataset.groupId === key) || null;
}

function captureTodayDetailsSnapshot(panel) {
  todaySearchSnapshot = Array.from(panel.querySelectorAll('details:not([data-search-ignore="true"])'))
    .map((detail) => {
      const key = getTodayDetailKey(detail);
      if (!key) return null;
      return {
        key,
        open: detail.open,
...
function restoreTodayDetailsSnapshot() {
  ...
  todaySearchSnapshot.forEach(({ key, open, display }) => {
    const detail = findTodayDetailByKey(panel, key);
    if (!detail) return;
    detail.open = open;
...
function countVisibleTodayCards(...scopes) {
  return scopes.reduce((total, scope) => (
    total + Array.from(scope?.querySelectorAll(".habit-card") || []).filter((card) => card.style.display !== "none").length
  ), 0);
}

function syncTodaySearchLayout(panel, hasQuery) {
  panel.querySelectorAll(".habit-today-accordion .habits-subgroup").forEach((subgroup) => {
    if (!hasQuery) {
      subgroup.style.display = "";
      return;
    }
    subgroup.style.display = countVisibleTodayCards(subgroup) ? "" : "none";
  });

  panel.querySelectorAll("#habits-today-results .empty-state").forEach((empty) => {
    if (hasQuery) {
      if (typeof empty.dataset.searchDisplay === "undefined") {
        empty.dataset.searchDisplay = empty.style.display || "";
      }
      empty.style.display = "none";
      return;
    }
    if (typeof empty.dataset.searchDisplay !== "undefined") {
      empty.style.display = empty.dataset.searchDisplay;
      delete empty.dataset.searchDisplay;
      return;
    }
    empty.style.display = "";
  });
  ...
  if ($habitCustomGroups) {
    if (!hasQuery) {
      $habitCustomGroups.style.display = $habitCustomGroups.childElementCount ? "" : "none";
      return;
    }
    const hasVisibleGroup = Array.from($habitCustomGroups.querySelectorAll('details.habit-custom-group[data-group-id]'))
      .some((detail) => detail.style.display !== "none");
    $habitCustomGroups.style.display = hasVisibleGroup ? "" : "none";
  }
}
...
function applyTodaySearch(value) {
  ...
  requestAnimationFrame(() => {
    ...
    syncTodaySearchLayout(panel, hasQuery);

    if (hasQuery) {
      const countPending = countVisibleTodayCards($habitTodayCountPending, $todayCountPendingPrivate, $todayCountPendingLow);
      const countDone = countVisibleTodayCards($habitTodayCountDone, $todayCountDonePrivate, $todayCountDoneLow);
      const timePending = countVisibleTodayCards($habitTodayTimePending, $todayTimePendingPrivate, $todayTimePendingLow);
      const timeDone = countVisibleTodayCards($habitTodayTimeDone, $todayTimeDonePrivate, $todayTimeDoneLow);
...
function renderHabitUnitSelector() {
  ...
  opts.forEach((opt) => {
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = `habit-unit-btn${habitDurationMode === opt.value ? " active" : ""}`;
...
function renderAnalyticsRangePanels() {
  ...
  if ($habitAccDonutBreakdownMeta) {
    $habitAccDonutBreakdownMeta.textContent = `${rangeLabelTitle(habitDonutRange)} · ${habitLastDonutBreakdown.length} segmentos`;
  }
  if ($habitDonutBreakdownList) {
    $habitDonutBreakdownList.innerHTML = "";
    if (!habitLastDonutBreakdown.length) {
      $habitDonutBreakdownList.innerHTML = '<div class="habit-range-empty">Sin datos en este rango.</div>';
    } else {
      habitLastDonutBreakdown.forEach((item, idx) => {
        const row = document.createElement("div");
        ...
        $habitDonutBreakdownList.appendChild(row);
      });
    }
  }

  if ($habitAccRangeHabitsMeta) {
    $habitAccRangeHabitsMeta.textContent = `${rangeLabelTitle(habitDonutRange)} · ${used.length}/${allHabits.length} activos`;
  }
  if ($habitRangeHabitsList) {
    $habitRangeHabitsList.innerHTML = "";
    if (!used.length) {
  ...
  if ($habitAccRangeUnusedTitle) $habitAccRangeUnusedTitle.textContent = `No usados (${unused.length})`;
  if ($habitRangeUnusedList) {
    $habitRangeUnusedList.innerHTML = "";
    if (!unused.length) {
      $habitRangeUnusedList.innerHTML = '<div class="habit-range-empty">Todos los hábitos tienen uso.</div>';
...
function renderQuickCounters() {
  ...
  if ($quickCounterCount) {
    $quickCounterCount.textContent = quickIds.length
      ? `${quickIds.length} fijado${quickIds.length === 1 ? "" : "s"}`
      : "—";
  }
...
function captureTodayUIState() {
  const panel = document.querySelector('.habits-panel[data-panel="today"]');
  const openIds = panel
    ? Array.from(panel.querySelectorAll("details[open]")).map((detail) => getTodayDetailKey(detail)).filter(Boolean)
    : [];
...
function restoreTodayUIState(state) {
  ...
  if (panel) {
    state.openIds.forEach((id) => {
      const detail = findTodayDetailByKey(panel, id);
...
async function stopSessionLegacy(assignHabitId = null, silent = false, sessionId = runningSession?.sessionId) {
  ...
  const session = sessionId ? activeSessions?.[sessionId] : null;
  if (!session) return;
  armSessionStopGuard("stop-flow");
  logHabitsSession("stop", {
    sessionId,
    habitId: session?.habitId || session?.targetHabitId || "",
    assignHabitId: typeof assignHabitId === "string" ? assignHabitId : "",
  });
...
function renderWorkScheduleRowsEditor() {
  if (!$habitWorkScheduleRows) return;
  $habitWorkScheduleRows.innerHTML = "";
  workScheduleEditor.rows.forEach((row) => {
    const item = document.createElement("div");
    item.className = "habit-work-schedule-row";
    const dayChips = WORK_SCHEDULE_DOWS.map((day) => `
      <button type="button" class="habit-work-schedule-day ${row.days.includes(day) ? "is-active" : ""}" data-row="${row.id}" data-day="${day}">${WORK_SCHEDULE_DOW_LABELS[day]}</button>
    `).join("");
    item.innerHTML = `
    ...
    `;
    $habitWorkScheduleRows.appendChild(item);
  });
}
...
function openWorkScheduleModal(options = {}) {
  ...
  $habitWorkScheduleModal.classList.remove("hidden");
  syncHabitModalOpenState();
}

function closeWorkScheduleModal() {
  $habitWorkScheduleModal?.classList.add("hidden");
  syncHabitModalOpenState();
}
...
function bindEvents() {
  ...
  const stopSessionButtonEvent = (event, explicitSessionId = null) => {
    event.preventDefault();
    event.stopPropagation();
    stopSession(null, false, explicitSessionId || undefined);
  };
  ...
  $habitOverlayStop.addEventListener("pointerdown", (event) => {
    event.preventDefault();
    event.stopPropagation();
  });
  $habitSessionDetailStop?.addEventListener("pointerdown", (event) => {
    event.preventDefault();
    event.stopPropagation();
  });
  $habitOverlayStop.addEventListener("click", (event) => stopSessionButtonEvent(event));
  $habitSessionDetailStop?.addEventListener("click", (event) => stopSessionButtonEvent(event));
  ...
  $habitSessionDetail?.addEventListener("click", (event) => {
    ...
    if (stopBtn) {
      stopSessionButtonEvent(event, stopBtn.getAttribute("data-session-stop"));
      return;
    }
  ...
  habitQuickPanel = loadHabitQuickPanel();
  const quickSessionsSummary = $habitQuickSessionsWrap?.querySelector("summary");
  quickSessionsSummary?.setAttribute("tabindex", "-1");
  quickSessionsSummary?.addEventListener("click", (event) => event.preventDefault());
  syncHabitQuickPanelUI();
  $habitQuickSessionsTab?.addEventListener("click", () => setHabitQuickPanel("sessions"));
  $habitQuickCountersTab?.addEventListener("click", () => setHabitQuickPanel("counters"));
  $habitQuickWorkSchedulesTab?.addEventListener("click", openWorkScheduleModal);

  $habitWorkScheduleClose?.addEventListener("click", closeWorkScheduleModal);
  $habitWorkScheduleCancel?.addEventListener("click", closeWorkScheduleModal);
  $habitWorkScheduleModal?.addEventListener("click", (event) => {
    if (event.target === $habitWorkScheduleModal) closeWorkScheduleModal();
  });
  ...
  $habitWorkScheduleAddRow?.addEventListener("click", () => {
    workScheduleEditor.rows.push(normalizeWorkRow({}));
    renderWorkScheduleRowsEditor();
  });
  $habitWorkScheduleRows?.addEventListener("click", (event) => {
    const timeInput = event.target?.closest?.(".habit-work-schedule-time-input");
    if (timeInput) {
      const segment = getWorkScheduleTimeSegmentFromCaret(timeInput, event.clientX);
      window.requestAnimationFrame(() => selectWorkScheduleTimeSegment(timeInput, segment));
      return;
    }
    const dayBtn = event.target?.closest?.("[data-day]");
    if (dayBtn) {
      const row = workScheduleEditor.rows.find((item) => item.id === dayBtn.dataset.row);
      if (!row) return;
      const day = dayBtn.dataset.day;
      if (!WORK_SCHEDULE_DOWS.includes(day)) return;
      if (row.days.includes(day)) row.days = row.days.filter((item) => item !== day);
      else row.days = [...row.days, day];
      renderWorkScheduleRowsEditor();
      return;
    }
    const removeBtn = event.target?.closest?.("[data-row-remove]");
    if (removeBtn) {
      workScheduleEditor.rows = workScheduleEditor.rows.filter((row) => row.id !== removeBtn.dataset.rowRemove);
      renderWorkScheduleRowsEditor();
    }
  });
  $habitWorkScheduleRows?.addEventListener("focusin", (event) => {
    const input = event.target?.closest?.(".habit-work-schedule-time-input");
    if (!input) return;
    ensureWorkScheduleTimeInputValue(input);
    const row = workScheduleEditor.rows.find((item) => item.id === input.dataset.row);
    if (row && (input.dataset.field === "start" || input.dataset.field === "end")) {
      row[input.dataset.field] = input.value;
    }
    window.requestAnimationFrame(() => selectWorkScheduleTimeSegment(input, "hours"));
  });
  $habitWorkScheduleRows?.addEventListener("input", (event) => {
...
function getHabitSessionTargetHabitIds() {
...
function bindRemoteValueListener(path, handler, { key, reason = "habits-live-sync", bounded = false, querySummary = "" } = {}) {
  ...
  const unsubscribe = trackedOnValue(ref(db, path), handler, {
    key: key || path,
    path,
    module: "habits",
    mode: "onValue",
...
function bindRemoteChildListeners(path, {
  ...
  attach("onChildAdded", () => onChildAdded(ref(db, path), (snap) => {
    ...
    trackFirebasePayload({
      ...
      source: "onChildAdded",
      value: snap.val(),
      key: `${keyPrefix}:onChildAdded`,
      eventIndex: eventCounters.onChildAdded,
      initial: eventCounters.onChildAdded === 1,
      bounded,
      querySummary,
      mode: "onChildAdded",
    });
  ...
  attach("onChildChanged", () => onChildChanged(ref(db, path), (snap) => {
    eventCounters.onChildChanged += 1;
    trackFirebasePayload({
      path,
      module: "habits",
      reason,
      viewId: "view-habits",
      source: "onChildChanged",
      value: snap.val(),
      key: `${keyPrefix}:onChildChanged`,
      eventIndex: eventCounters.onChildChanged,
      initial: false,
      bounded,
      querySummary,
      mode: "onChildChanged",
  ...
  attach("onChildRemoved", () => onChildRemoved(ref(db, path), (snap) => {
    eventCounters.onChildRemoved += 1;
    trackFirebasePayload({
      path,
      module: "habits",
      reason,
      viewId: "view-habits",
      source: "onChildRemoved",
      value: snap.val(),
      key: `${keyPrefix}:onChildRemoved`,
      eventIndex: eventCounters.onChildRemoved,
      initial: false,
      bounded,
      querySummary,
      mode: "onChildRemoved",
...
function getHabitSessionTailBounds(days = 45) {
  const end = endOfDay(new Date());
  const span = Math.max(1, Number(days) || 45);
  const start = addDays(end, -(span - 1));
  return { start, end, limit: span };
}

function buildHabitSessionRangeSpec(mode, start, end, extra = {}) {
  const safeStart = start instanceof Date && !Number.isNaN(start.getTime())
    ? new Date(start.getFullYear(), start.getMonth(), start.getDate())
    : new Date();
  const safeEnd = end instanceof Date && !Number.isNaN(end.getTime())
    ? endOfDay(end)
    : endOfDay(new Date());
  const normalizedStart = safeStart <= safeEnd ? safeStart : new Date(safeEnd.getFullYear(), safeEnd.getMonth(), safeEnd.getDate());
  const startKey = dateKeyLocal(normalizedStart);
  const endKey = dateKeyLocal(safeEnd);
  return {
    mode,
...
function buildHabitReportsSessionSpec() {
  const windows = [];
  const fullReasons = new Set();
  const pushWindow = (rangeLabel, bounds) => {
    if (!bounds?.start || !bounds?.end) return;
    windows.push({ rangeLabel, ...buildHabitSessionRangeSpec("range-get", bounds.start, bounds.end) });
  };

  if (habitDonutRange === "total") fullReasons.add("reports-donut-total");
  else pushWindow(`donut:${habitDonutRange}`, getRangeBounds(habitDonutRange));

  if (habitRecordsRange === "total") fullReasons.add("reports-records-total");
  else pushWindow(`records:${habitRecordsRange}`, getRangeBounds(habitRecordsRange));

  if (habitLineRange === "total") fullReasons.add("reports-line-total");
  else pushWindow(`line:${habitLineRange}`, getLineRangeBounds(habitLineRange));

  pushWindow(`heatmap:${heatmapYear}`, {
    start: new Date(heatmapYear, 0, 1),
    end: new Date(heatmapYear, 11, 31, 23, 59, 59, 999),
  });
  ...
  if (fullReasons.size) {
    return {
      mode: "full-get",
      reasonLabel: Array.from(fullReasons).join(","),
      startKey: "",
      endKey: "",
    };
  }
  ...
  if (!windows.length) {
    const tail = getHabitSessionTailBounds(45);
    return buildHabitSessionRangeSpec("range-get", tail.start, tail.end, {
      reasonLabel: "reports-fallback-tail",
    });
  }

  const start = windows.reduce((min, entry) => (entry.start < min ? entry.start : min), windows[0].start);
  const end = windows.reduce((max, entry) => (entry.end > max ? entry.end : max), windows[0].end);
  return buildHabitSessionRangeSpec("range-get", start, end, {
    reasonLabel: windows.map((entry) => entry.rangeLabel).join(","),
  });
}

function buildHabitSessionSyncSpec() {
  if (!currentUid || !isHabitsViewVisible()) {
    return { mode: "none", key: "none", bounded: true, reasonLabel: "view-hidden", habitIds: [] };
  }

  const habitIds = getHabitSessionTargetHabitIds();
  if (!habitIds.length) {
    return { mode: "empty", key: "empty", bounded: true, reasonLabel: "no-habits", habitIds };
  }

  let spec;
  if (activeTab === "history") {
    const bounds = getHistoryRangeBounds(habitHistoryRange);
    spec = buildHabitSessionRangeSpec(habitHistoryRange === "total" ? "full-get" : "range-get", bounds.start, bounds.end, {
      reasonLabel: `history:${habitHistoryRange}`,
    });
  } else if (activeTab === "reports") {
    spec = buildHabitReportsSessionSpec();
  } else {
    const bounds = getHabitSessionTailBounds(activeTab === "schedule" ? 60 : 45);
    spec = buildHabitSessionRangeSpec("live-tail", bounds.start, bounds.end, {
      limit: bounds.limit,
      reasonLabel: `${activeTab}:tail-${bounds.limit}d`,
    });
  }

  const bounded = spec.mode !== "full-get";
  const detailKey = spec.mode === "full-get"
    ? spec.reasonLabel
    : `${spec.startKey}:${spec.endKey}:${spec.limit || 0}`;
  return {
    ...spec,
    habitIds,
    bounded,
    key: `${activeTab}:${spec.mode}:${detailKey}:${habitIds.join(",")}`,
  };
}

function stopHabitSessionLiveSync() {
  habitSessionLiveUnsubscribers.forEach((unsubscribe) => {
    try {
      unsubscribe();
    } catch (_) {}
  });
  habitSessionLiveUnsubscribers = [];
  habitSessionLiveSlices = new Map();
}

function applyHabitSessionSyncSnapshot(rawByHabit, reason = "remote:sessions", spec = null) {
  const nextRaw = rawByHabit && typeof rawByHabit === "object" ? rawByHabit : {};
  applyPendingSessionWrites(nextRaw);
  const norm = normalizeSessionsStore(nextRaw, true);
  habitSessions = norm.normalized;
  habitSessionTimeline = norm.timeline || {};
  habitSessionTimelineCoverage = norm.coverage || {};
  debugHabitsSync("habitSessions sync applied", {
    reason,
    mode: spec?.mode || "",
    key: spec?.key || "",
  ...
  markHistoryDataChanged(reason, spec ? {
    mode: spec.mode,
    key: spec.key,
    startKey: spec.startKey || "",
    endKey: spec.endKey || "",
...
async function syncHabitSessionsForCurrentView({ force = false, reason = "unknown" } = {}) {
  ...
  const syncVersion = habitSessionSyncVersion;
  habitSessionSyncMode = spec.mode;
  habitSessionSyncKey = spec.key;

  if (spec.mode !== "live-tail") {
    stopHabitSessionLiveSync();
  }

  if (spec.mode === "none" || spec.mode === "empty") {
    applyHabitSessionSyncSnapshot({}, `remote:sessions:${reason}`, spec);
    return;
  }
  ...
  if (spec.mode === "live-tail") {
    ...
    spec.habitIds.forEach((habitId) => {
      const path = `${HABIT_SESSIONS_PATH}/${habitId}`;
      ...
      const unsubscribe = trackedOnValue(
        sessionsQuery,
        (snap) => {
          if (syncVersion !== habitSessionSyncVersion) return;
          nextSlices.set(habitId, snap.val() || {});
          const nextRaw = {};
          nextSlices.forEach((value, key) => {
            if (value && typeof value === "object" && Object.keys(value).length) {
              nextRaw[key] = value;
            }
          });
          habitSessionLiveSlices = nextSlices;
          applyHabitSessionSyncSnapshot(nextRaw, `remote:sessions:${spec.reasonLabel}`, spec);
        },
        {
        key: `habit-sessions:${habitId}`,
        path,
        module: "habits",
        mode: "onValue",
        reason: `habits-session-sync:${spec.reasonLabel}`,
        viewId: "view-habits",
        bounded: true,
        querySummary: `orderByKey,startAt(${spec.startKey}),limitToLast(${spec.limit || 45})`,
        onError: (error) => {
          console.warn("No se pudo escuchar sesiones de hábitos", { habitId, error });
        },
      },
      onValue);
      if (typeof unsubscribe === "function") {
  ...
  const nextRaw = {};
  const useFullHistory = spec.mode === "full-get";
  if (useFullHistory) {
    logFirebaseBytesRisk({
      path: HABIT_SESSIONS_PATH,
      reason: `habits-session-sync:${spec.reasonLabel || reason}`,
      viewId: "view-habits",
      estimatedCount: spec.habitIds.length,
    });
  }

  await Promise.all(spec.habitIds.map(async (habitId) => {
    const path = `${HABIT_SESSIONS_PATH}/${habitId}`;
    const requestRef = useFullHistory
      ? ref(db, path)
      : query(ref(db, path), orderByKey(), startAt(spec.startKey), endAt(spec.endKey));
    const snap = await trackedGet(requestRef, {
      path,
      module: "habits",
      reason: `habits-session-sync:${spec.reasonLabel}`,
      viewId: "view-habits",
      bounded: !useFullHistory,
      querySummary: useFullHistory
        ? "full-history-per-habit"
        : `orderByKey,startAt(${spec.startKey}),endAt(${spec.endKey})`,
    }, get);
    if (syncVersion !== habitSessionSyncVersion) return;
    const value = snap?.val() || {};
    if (value && typeof value === "object" && Object.keys(value).length) {
      nextRaw[habitId] = value;
    }
  }));

  if (syncVersion !== habitSessionSyncVersion) return;
  applyHabitSessionSyncSnapshot(nextRaw, `remote:sessions:${spec.reasonLabel || reason}`, spec);
}
...
function listenRemoteLegacy() {
  ...
  const bindRemote = (path, handler) => {
    logFirebaseRead({ path, mode: "onValue", reason: "habits-live-sync", viewId: "view-habits" });
  ...
  bindRemote(HABITS_PATH, (snap) => {
    ...
    saveCache();
    void syncHabitSessionsForCurrentView({ force: true, reason: "remote:habits-legacy" });
    rerender();
  });
  ...
  bindRemote(HABIT_WORK_SCHEDULES_PATH, (snap) => {
    habitWorkSchedules = normalizeWorkSchedulesStore(snap.val() || {});
    saveCache();
  ...
  remoteBound = true;
  void syncHabitSessionsForCurrentView({ force: true, reason: "listen-remote-legacy" });
}
...
async function startSession(habitId = null, meta = null) {
  if (!currentUid || !HABIT_ACTIVE_SESSIONS_PATH) return null;
  const requestedHabitId = typeof habitId === "string" ? String(habitId || "").trim() : "";
  const targetHabitId = (typeof habitId === "string" && habits?.[habitId] && !habits[habitId]?.archived)
    ? habitId
    : null;
  if (!targetHabitId && isSessionStopGuardActive()) {
    logHabitsSession("generic-create blocked", {
      reason: sessionStopGuardReason || "stop-flow",
      requestedHabitId,
    });
    logHabitsSession("unexpected-autostart blocked", {
      reason: sessionStopGuardReason || "stop-flow",
      requestedHabitId,
    });
    return null;
...
async function stopSession(assignHabitId = null, silent = false, sessionId = runningSession?.sessionId) {
  ...
  logHabitsSession("stop:success", {
    ...
    habitId: target || "",
    durationSec: duration,
    requiresAssignment: !(target && habits?.[target] && !habits[target]?.archived),
  });
...
function listenRemoteDeprecated() {
  ...
  bindRemote(HABITS_PATH, (snap) => {
    ...
    saveCache();
    void syncHabitSessionsForCurrentView({ force: true, reason: "remote:habits" });
    rerender();
  });
...
function goHabitSubtab(tab) {
  ...
  void syncHabitSessionsForCurrentView({ force: true, reason: `tab:${activeTab}` });
  renderHabits();
}
