...
const state = { initialized:false, unsub:null, rootRef:null, map:null, miniMap:null, miniMarkers:[], markers:new Map(), geography:[], places:[], userCenter:{ lat:DEFAULT_MAP_CENTER_SPAIN[0], lon:DEFAULT_MAP_CENTER_SPAIN[1] }, selectedGeo:null, selectedPlace:null, selectedGeoIndex:-1, selectedPlaceIndex:-1, geoResults:[], placeResults:[], geoRating:0, placeRating:0, activeWindow:"map", addMode:"geo", editing:null, toastTimer:null };
...
function setAddMode(mode){ state.addMode = mode; $id("world-add-mode-geo").classList.toggle("active", mode === "geo"); $id("world-add-mode-place").classList.toggle("active", mode === "place"); if (mode === "geo") renderGeoAddMode(); else renderLocalAddMode(); }
...
function renderPlaceResults(){ const root = $id("world-place-results"); root.innerHTML = state.placeResults.map((r, i) => `<button class="world-result-card ${state.selectedPlaceIndex===i?"is-selected":""}" data-place-pick="${i}"><strong>${r.name}</strong><small>${r.addressLine || r.label || ""}${r.city ? `, ${r.city}` : ""} · ${Math.round(r.distance)} m</small></button>`).join(""); renderMiniMapMarkers(); }
...
function renderGeoList(){ const groups = state.geography.reduce((acc, r) => { const key = r.countryCode || r.country || "XX"; acc[key] = acc[key] || { country:r.country, countryCode:r.countryCode, rows:[] }; acc[key].rows.push(r); return acc; }, {}); const html = Object.values(groups).map((g)=>`<details open><summary><strong>${flag(g.countryCode)} ${g.country || getCountryEnglishName(g.countryCode) || ""}</strong> · ${g.rows.length}</summary>${g.rows.map((r)=>`<div class="world-rich-item"><div><div>${stars10(r.rating || 0)} ${(r.rating ?? 0).toFixed(1)}/10</div><small>• ${r.name || r.city}</small></div><div class="world-item-actions"><button data-world-center="${r.id}">📍</button><button data-world-edit="geography:${r.id}">✏️</button><button data-world-delete="geography:${r.id}">🗑</button></div></div>`).join("")}</details>`).join(""); $id("world-country-list").innerHTML = html || "<div>Sin lugares geográficos.</div>"; }
...
async function searchPlace(q){ const c=state.userCenter; const radius=Number($id("world-place-radius").value||1000); const dLat=radius/111320, dLng=radius/(111320*Math.cos((c.lat*Math.PI)/180)); const u=new URL("https://nominatim.openstreetmap.org/search"); u.searchParams.set("q",q); u.searchParams.set("format","jsonv2"); u.searchParams.set("addressdetails","1"); u.searchParams.set("limit","24"); u.searchParams.set("bounded","1"); u.searchParams.set("viewbox",`${c.lon-dLng},${c.lat+dLat},${c.lon+dLng},${c.lat-dLat}`); const rows=await (await fetch(u)).json(); state.placeResults=(Array.isArray(rows)?rows:[]).map((raw)=>{ const r=normalizeLocationAddress(raw); const addressLine = [r.address?.road, r.address?.house_number].filter(Boolean).join(" ") || [r.address?.suburb, r.address?.county].filter(Boolean).join(", "); return ({ ...r, addressLine, distance:hav(c.lat,c.lon,Number(r.lat),Number(r.lon)) }); }).filter((r)=>r.distance<=radius).sort((a,b)=>a.distance-b.distance); state.selectedPlace = null; state.selectedPlaceIndex = -1; renderPlaceResults(); }
