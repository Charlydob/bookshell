...
import { initWorldStays, renderWorldStays, openWorldStayModal, closeWorldStayModal } from "./stays.js";
...
function setWindow(mode){ state.activeWindow = mode; state.activeSubtab = mode; document.querySelectorAll("#view-world .world-tab").forEach((b)=>b.classList.toggle("active", b.dataset.window===mode)); document.querySelectorAll("#view-world [data-window-panel]").forEach((p)=>p.classList.toggle("active", p.dataset.windowPanel===mode)); const addBtn = $id("world-open-add"); if (addBtn) { if (mode === "stays") addBtn.dataset.worldAction = "add-stay"; else delete addBtn.dataset.worldAction; } invalidateLeafletMap(state.map, 60); }
