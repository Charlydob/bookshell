...
const WORLD_PATH = (authUid) => firebasePaths.world(authUid);
const LEGACY_WORLD_PATHS = (authUid) => [firebasePaths.legacyWorldTrips(authUid)];
...
const authUid = () => getCurrentUserDataRootKey() || worldState.firebaseAuthUid;
...
export async function init() {
  ...
  async function persistRemoteNow() { if (!authUid() || !worldState.firebaseRef) return; try { await update(worldState.firebaseRef, worldPatchPayload(state)); } catch (e) { console.warn("[world] remote save failed", e); } }
  ...
  async function initFirebaseSync() {
    if (!authUid()) return;
    worldState.firebaseAuthUid = authUid();
    worldState.firebaseRef = ref(db, WORLD_PATH(authUid()));
    const legacy = await readLegacyWorldPayload(authUid());
    logFirebaseRead({ path: WORLD_PATH(authUid()), mode: "onValue", reason: "world-live-sync", viewId: "view-world" });
