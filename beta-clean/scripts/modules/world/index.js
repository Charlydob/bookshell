...
function renderLocals(){ const rows=state.places; const mode = $id("world-locals-group")?.value || "type"; const statsMount = $id("world-locals-stats"); if (statsMount) { const byCountry = new Map(), byCity = new Map(), byCategory = new Map(), byChain = new Map(); const ratings = rows.filter((r)=>Number.isFinite(Number(r.rating))); rows.forEach((r)=>{ byCountry.set(r.country || "Sin país", (byCountry.get(r.country || "Sin país") || 0) + 1); byCity.set(r.city || "Sin ciudad", (byCity.get(r.city || "Sin ciudad") || 0) + 1); byCategory.set(r.category || "Sin categoría", (byCategory.get(r.category || "Sin categoría") || 0) + 1); const chain = normalizeChainName(r); byChain.set(chain, (byChain.get(chain) || 0) + 1); }); const top=(m)=>Array.from(m.entries()).sort((a,b)=>b[1]-a[1]).slice(0,5); const avgGlobal = ratings.length ? ratings.reduce((s, r) => s + Number(r.rating || 0), 0) / ratings.length : 0; statsMount.innerHTML = `<details class="world-local-stats-box"><summary>Estadísticas de locales ▾</summary><div class="world-local-stats-grid"><div class="world-pill">Total locales: ${rows.length}</div><div class="world-pill">Valoración media global: ${avgGlobal.toFixed(1)}/10</div><div><strong>Top países</strong>${top(byCountry).map(([k,v])=>`<div class="world-stat-row"><span>${k}</span><b>${v}</b></div>`).join("")}</div><div><strong>Top ciudades</strong>${top(byCity).map(([k,v])=>`<div class="world-stat-row"><span>${k}</span><b>${v}</b></div>`).join("")}</div><div><strong>Top categorías</strong>${top(byCategory).map(([k,v])=>`<div class="world-stat-row"><span>${k}</span><b>${v}</b></div>`).join("")}</div><div><strong>Top cadenas</strong>${top(byChain).map(([k,v])=>`<div class="world-stat-row"><span>${k}</span><b>${v}</b></div>`).join("")}</div></div></details>`; } $id("world-locals-list").innerHTML = rows.map((r)=>`<div class="world-rich-item"><div class="world-local-card"><div class="world-local-line world-local-line-main"><span>${r.emoji || "🏪"}</span><span class="world-local-name">${toTitleCase(r.name || r.category || "Local")}</span><span class="world-local-dot">·</span><span>${r.city || r.region || "Sin ciudad"}</span></div><div class="world-local-line"><span>${stars10(r.rating || 0)} ${(r.rating ?? 0).toFixed(1)}/10</span></div></div><div class="world-item-actions"><button data-world-center="${r.id}">📍</button><button data-world-edit="places:${r.id}">✏️</button><button data-world-rate="places:${r.id}">⭐</button><button data-world-delete="places:${r.id}">🗑</button></div></div>`).join("") || "<div>Sin locales.</div>"; }
