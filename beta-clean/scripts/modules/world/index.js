...
const LOCAL_CATEGORY_EMOJI_STORAGE_KEY = "worldLocalCategoryEmojis";
const DEFAULT_LOCAL_CATEGORY_EMOJIS = [
  { category:"Montaña", emoji:"🏔️" },
  { category:"Restaurante", emoji:"🍽️" },
  { category:"Cafetería", emoji:"☕" },
  { category:"Farmacia", emoji:"💊" },
  { category:"Hotel", emoji:"🏨" },
  { category:"Supermercado", emoji:"🛒" },
  { category:"Museo", emoji:"🏛️" },
  { category:"Playa", emoji:"🏖️" },
  { category:"Parque", emoji:"🌳" },
  { category:"Monumento", emoji:"🗿" },
  { category:"Transporte", emoji:"🚉" },
  { category:"Otro", emoji:"📍" }
];
const LOCAL_CATEGORY_KEYWORD_EMOJIS = [
  { emoji:"🏔️", keywords:["montana","mountain","sierra","cerro"] },
  { emoji:"🍕", keywords:["pizzeria","pizza"] },
  { emoji:"☕", keywords:["cafeteria","cafe","coffee"] },
  { emoji:"🍽️", keywords:["restaurante","restaurant","comida"] },
  { emoji:"💊", keywords:["farmacia","pharmacy"] },
  { emoji:"🏨", keywords:["hotel","hostel"] },
  { emoji:"🛒", keywords:["supermercado","supermarket","market"] },
  { emoji:"🏛️", keywords:["museo","museum"] },
  { emoji:"🏖️", keywords:["playa","beach"] },
  { emoji:"🌳", keywords:["parque","park","jardin"] },
  { emoji:"🗿", keywords:["monumento","monument"] },
  { emoji:"🚉", keywords:["transporte","estacion","station","metro","tren"] },
  { emoji:"🍸", keywords:["bar","pub","coctel"] },
...
function normalizeEmojiAssociation(entry = {}) {
  const category = String(entry.category || entry.name || "").trim();
  const emoji = String(entry.emoji || "").trim();
  const key = normalizeCategoryEmojiKey(entry.key || category);
  if (!key || !category || !emoji) return null;
  return { key, category, emoji };
}
function getDefaultLocalCategoryEmojiMap() {
  return Object.fromEntries(DEFAULT_LOCAL_CATEGORY_EMOJIS.map((entry) => [normalizeCategoryEmojiKey(entry.category), { ...entry, key:normalizeCategoryEmojiKey(entry.category) }]));
}
function readLocalCategoryEmojis() {
  try {
    const raw = JSON.parse(localStorage.getItem(LOCAL_CATEGORY_EMOJI_STORAGE_KEY) || "{}");
    return Object.fromEntries(Object.values(raw || {}).map((entry) => normalizeEmojiAssociation(entry)).filter(Boolean).map((entry) => [entry.key, entry]));
  } catch (_error) {
    return {};
  }
}
function writeLocalCategoryEmojis(map = {}) {
  try { localStorage.setItem(LOCAL_CATEGORY_EMOJI_STORAGE_KEY, JSON.stringify(map || {})); } catch (_error) {}
}
function getCategoryEmojiAssociations({ includePlaces = true } = {}) {
  const associations = { ...getDefaultLocalCategoryEmojiMap(), ...readLocalCategoryEmojis(), ...(state.localCategoryEmojis || {}) };
  if (includePlaces) {
    state.places.forEach((place) => {
      const category = String(place.category || place.type || "").trim();
      const emoji = String(place.emoji || "").trim();
      const key = normalizeCategoryEmojiKey(category);
      if (key && category && emoji && !associations[key]) associations[key] = { key, category, emoji };
    });
  }
  return associations;
}
function persistLocalCategoryEmojiLocal(entry = {}) {
  const normalized = normalizeEmojiAssociation(entry);
  if (!normalized) return null;
  const localMap = { ...readLocalCategoryEmojis(), [normalized.key]:normalized };
  writeLocalCategoryEmojis(localMap);
  state.localCategoryEmojis = { ...(state.localCategoryEmojis || {}), [normalized.key]:normalized };
  return normalized;
}
async function persistLocalCategoryEmoji(entry = {}) {
  const normalized = persistLocalCategoryEmojiLocal(entry);
  if (!normalized) return null;
  const uid = getCurrentUserDataRootKey() || auth.currentUser?.uid;
  if (uid && firebasePaths.worldCategoryEmojis) await set(ref(db, firebasePaths.worldCategoryEmojis(uid)), state.localCategoryEmojis || {});
  return normalized;
}
async function ensureLocalCategoryEmoji(category = "", emoji = "", { persistRemote = true } = {}) {
  const normalized = normalizeEmojiAssociation({ category, emoji });
  if (!normalized) return null;
  const existing = getCategoryEmojiAssociations({ includePlaces:false })[normalized.key];
  if (existing) return existing;
  return persistRemote ? persistLocalCategoryEmoji(normalized) : persistLocalCategoryEmojiLocal(normalized);
}
function migrateLocalCategoryEmojisFromPlaces(rawPlaces = []) {
  let changed = false;
  rawPlaces.forEach((place) => {
    const category = String(place?.category || place?.type || "").trim();
    const emoji = String(place?.emoji || "").trim();
    const key = normalizeCategoryEmojiKey(category);
    if (!category || !emoji || !key || getCategoryEmojiAssociations({ includePlaces:false })[key]) return;
    if (persistLocalCategoryEmojiLocal({ category, emoji })) changed = true;
  });
  return changed;
}
function inferLocalCategoryEmoji(categoryOrType = "") {
  const normalized = normalizeCategoryEmojiKey(categoryOrType);
  if (!normalized) return "📍";
  const match = LOCAL_CATEGORY_KEYWORD_EMOJIS.find((entry) => entry.keywords.some((keyword) => normalized.includes(keyword)));
  return match?.emoji || "📍";
}
function getLocalCategoryEmoji(categoryOrType = "") {
  const category = String(categoryOrType || "").trim();
  const key = normalizeCategoryEmojiKey(category);
  if (!key) return "📍";
  const association = getCategoryEmojiAssociations()[key];
  return String(association?.emoji || inferLocalCategoryEmoji(category) || "📍").trim() || "📍";
}
function getLocalCategoryDisplayName(categoryOrType = "") {
  const category = String(categoryOrType || "").trim();
  const key = normalizeCategoryEmojiKey(category);
  const association = getCategoryEmojiAssociations()[key];
  return String(association?.category || category || "Otro").trim();
}
function getWorldMarkerEmoji(item = {}) {
  const category = String(item?.category || item?.type || "").trim();
  if (category) return getLocalCategoryEmoji(category);
  return String(item?.emoji || "").trim() || "📍";
}
function syncLocalCategoryEmojiPreview(scope = "add") {
  const categoryInput = $id(scope === "edit" ? "world-place-edit-category" : "world-place-form-category");
  const emojiInput = $id(scope === "edit" ? "world-place-edit-emoji" : "world-place-form-emoji");
  const preview = $id(scope === "edit" ? "world-place-edit-emoji-preview" : "world-place-form-emoji-preview");
  const hint = $id(scope === "edit" ? "world-place-edit-emoji-hint" : "world-place-form-emoji-hint");
  const category = String(categoryInput?.value || "").trim();
  const key = normalizeCategoryEmojiKey(category);
  const association = key ? getCategoryEmojiAssociations()[key] : null;
  if (emojiInput && !association && emojiInput.readOnly && emojiInput.dataset.categoryEmojiKey !== key) emojiInput.value = "";
  const emoji = association?.emoji || String(emojiInput?.value || "").trim() || inferLocalCategoryEmoji(category);
  if (preview) preview.textContent = emoji || "📍";
  if (emojiInput) {
    emojiInput.value = association?.emoji || (emojiInput.value || "");
    emojiInput.readOnly = Boolean(association);
    emojiInput.dataset.categoryEmojiKey = key;
    emojiInput.placeholder = association ? "Emoji asociado" : "Emoji para esta categoría";
  }
  if (hint) hint.textContent = association ? `Emoji vinculado a ${association.category}` : "Nueva categoría: asigna un emoji una sola vez.";
  return { category, emoji:emoji || "📍", association };
}
const normalize = (r = {}, kind = "geography") => ({ id:r.id || id(), kind, name:String(r.name || r.placeName || "").trim(), label:String(r.label || r.displayName || r.name || "").trim(), displayName:String(r.displayName || r.label || r.name || "").trim(), country:String(r.country || "").trim(), countryCode:String(r.countryCode || r.country_code || "").trim().toUpperCase(), city:String(r.city || "").trim(), region:String(r.region || r.subdivision || "").trim(), postalCode:String(r.postalCode || "").trim(), address:String(r.address || "").trim(), category:String(r.category || r.type || "").trim(), emoji:String(kind === "places" ? (String(r.category || r.type || "").trim() ? getLocalCategoryEmoji(r.category || r.type || "") : (r.emoji || "📍")) : (r.emoji || "📍")).trim(), note:String(r.note || "").trim(), productName:String(r.productName || "").trim(), price:Number.isFinite(Number(r.price)) ? Number(r.price) : null, currency:String(r.currency || "EUR").trim().toUpperCase(), rating:Number.isFinite(Number(r.rating)) ? Number(r.rating) : null, lat:Number(r.lat), lon:Number(r.lon ?? r.lng), lng:Number(r.lng ?? r.lon), googleMapsDirectionsUrl:String(r.googleMapsDirectionsUrl || r.googleMapsUrl || "").trim(), createdAt:Number(r.createdAt || Date.now()), updatedAt:Date.now() });
const toTitleCase = (s = "") => {
  const lower = String(s || "").trim().toLocaleLowerCase("es");
  return lower.replace(/(^|[\s\-_/()"'«»“”.,;:!?¿¡])(\p{L})/gu, (_, sep, letter) => `${sep}${letter.toLocaleUpperCase("es")}`);
};
const canonicalWorldValue = (value = "") => String(value || "").trim().toLocaleLowerCase("es").normalize("NFC");
function getReusableLocalValues() {
  const associations = getCategoryEmojiAssociations();
  const categories = new Map();
  Object.values(associations).forEach((entry) => {
    if (entry?.key && entry?.category) categories.set(entry.key, entry.category);
  });
  state.places.forEach((place) => {
    const cat = String(place.category || place.type || "").trim();
    const catKey = normalizeCategoryEmojiKey(cat);
    if (catKey && !categories.has(catKey)) categories.set(catKey, cat);
  });
  return { categories:Array.from(categories.values()).sort((a, b) => a.localeCompare(b, "es")) };
}
function renderReusableLocalSelectors() {
  const categorySelect = $id("world-place-form-category-select");
  const categoryDatalist = $id("world-place-form-category-options");
  const editCategoryDatalist = $id("world-place-edit-category-options");
  if (!categorySelect) return;
  const { categories } = getReusableLocalValues();
  const options = categories.map((cat) => `<option value="${cat}">${getLocalCategoryEmoji(cat)} ${cat}</option>`).join("");
  categorySelect.innerHTML = `<option value="">Elegir categoría</option>${options}`;
  if (categoryDatalist) categoryDatalist.innerHTML = options;
  if (editCategoryDatalist) editCategoryDatalist.innerHTML = options;
  syncLocalCategoryEmojiPreview("add");
  syncLocalCategoryEmojiPreview("edit");
}
...
function getWorldLocalCategories(){
  ...
  state.places.forEach((item) => {
    ...
    if (!key) return;
    if (!categories.has(key)) categories.set(key, { id:key, name:toTitleCase(item.categoryName || item.category || item.type || item.name || "Sin categoría"), emoji:getLocalCategoryEmoji(item.category || item.type || item.categoryName || "") });
  });
...
function getMostFrequentGroupEmoji(rows = []){
  ...
  rows.forEach((row) => {
    const emoji = getWorldMarkerEmoji(row);
    if (!emoji) return;
...
function openEditPlaceModal(placeId){
  ...
  $id("world-place-edit-emoji").value = getWorldMarkerEmoji(rec);
  syncLocalCategoryEmojiPreview("edit");
...
function pickPlace(i, opts = {}){
  ...
  $id("world-place-form-category").value = state.selectedPlace.category || ""; $id("world-place-form-emoji").value = getWorldMarkerEmoji(state.selectedPlace); syncLocalCategoryEmojiPreview("add"); $id("world-place-form-country").value = state.selectedPlace.country || ""; $id("world-place-form-region").value = state.selectedPlace.region || ""; $id("world-place-form-city").value = state.selectedPlace.city || ""; if (state.miniMap) state.miniMap.setView([row.lat, row.lon], 17); renderPlaceResults();
}
...
async function useCurrentLocationForGeoPlace(){
  console.log("[world:add-place:location] start");
  try {
    const pos = await getCurrentPositionSafe({ forceRequest:true });
    if (!pos || !Number.isFinite(pos.lat) || !Number.isFinite(pos.lon)) throw new Error("position-unavailable");
    const reverse = await reverseGeocode(pos.lat, pos.lon);
    const fallbackAddress = reverse.address || { country:reverse.country, country_code:reverse.countryCode, state:reverse.region, city:reverse.city || reverse.municipality };
    state.geoResults = [{ ...reverse, address:fallbackAddress, lat:pos.lat, lon:pos.lon, label:reverse.label || reverse.name || "Ubicación actual", name:reverse.name || reverse.city || "Ubicación actual" }];
    pickGeo(0);
    renderGeoResults();
    console.log("[world:add-place:location] success", { lat:pos.lat, lon:pos.lon });
  } catch (error) {
    console.log("[world:add-place:location] error", error);
    showToast("No se pudo obtener ubicación actual");
  }
}
...
function bindUI(){
  ...
  root.addEventListener("click", async (e)=>{ const t=e.target.closest("button,label"); if(!t) return; if(t.matches(".world-tab")) return setWindow(t.dataset.window); if(t.id==="world-open-add") { $id("world-add-toggle").checked=true; setAddMode("geo"); resetWorldAddModalState(); state.showEditLocationSearch = true; state.placeModalMode = "add"; setEditModeUI(); }
    ...
    if(t.id==="world-geo-use-current-location") { await useCurrentLocationForGeoPlace(); }
  ...
  root.addEventListener("change", (e)=>{ if(e.target?.id==="world-map-stats-filter"){ state.mapStatsCountry = e.target.value; renderGeoList(); } if (e.target?.id === "world-map-category-filter-select" && e.target.value) { toggleWorldMapCategoryFilter(e.target.value); e.target.value = ""; } if (e.target?.id === "world-place-form-category-select" && e.target.value) { $id("world-place-form-category").value = e.target.value; console.log("[world:add-local:reuse-category]", e.target.value); syncLocalCategoryEmojiPreview("add"); }});
  root.addEventListener("input", (e)=>{ if (["world-place-form-category", "world-place-form-emoji"].includes(e.target?.id)) syncLocalCategoryEmojiPreview("add"); if (["world-place-edit-category", "world-place-edit-emoji"].includes(e.target?.id)) syncLocalCategoryEmojiPreview("edit"); });
  ...
  $id("world-place-save").addEventListener("click", async ()=>{ if (state.editing) return saveEdit(); if(!state.selectedPlace) return; const preview = syncLocalCategoryEmojiPreview("add"); const categorySaved = preview.association?.category || preview.category; const emojiSaved = preview.emoji || "📍"; await ensureLocalCategoryEmoji(categorySaved, emojiSaved); state.selectedPlace.rating = state.placeRating; state.selectedPlace.note = $id("world-place-note").value.trim(); state.selectedPlace.name = $id("world-place-form-name").value.trim() || state.selectedPlace.name; state.selectedPlace.category = categorySaved; state.selectedPlace.emoji = getLocalCategoryEmoji(categorySaved); state.selectedPlace.country = $id("world-place-form-country").value.trim() || state.selectedPlace.country; state.selectedPlace.region = $id("world-place-form-region").value.trim() || state.selectedPlace.region; state.selectedPlace.city = $id("world-place-form-city").value.trim() || state.selectedPlace.city; state.selectedPlace.productName = $id("world-place-form-product").value.trim(); state.selectedPlace.price = parsePrice($id("world-place-form-price").value); state.selectedPlace.currency = "EUR"; state.selectedPlace.lng = Number(state.selectedPlace.lon); state.selectedPlace.googleMapsDirectionsUrl = createGoogleMapsUrl(state.selectedPlace.lat, state.selectedPlace.lon); state.selectedPlace.googleMapsUrl = state.selectedPlace.googleMapsDirectionsUrl; if ((!String(state.selectedPlace.name || "").trim() || String(state.selectedPlace.name || "").trim() === "Nuevo local") && String(state.selectedPlace.address || "").trim()) state.selectedPlace.name = state.selectedPlace.address; if (!String(state.selectedPlace.label || "").trim() && String(state.selectedPlace.address || "").trim()) state.selectedPlace.label = state.selectedPlace.address; console.debug("[world:save:local:start]", state.selectedPlace); state.places.push(state.selectedPlace); await persistWorld(); console.debug("[world:save:local:stored]", state.selectedPlace); renderAll(); closeAddModal(); resetWorldAddModalState(); showToast("Guardado"); });
