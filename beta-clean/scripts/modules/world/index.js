...
import { ref, get, onValue, set, update, remove } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-database.js";
...
function computeStaySummary(stays = []){
  ...
  const summary = Object.values(byCountry).map((country) => ({ ...country, cities:Object.entries(country.cities).map(([city, daysTotal]) => ({ city, daysTotal })) }));
  console.debug("[world:stays:summary]", summary);
  return summary;
}
function renderStays(){
  ...
  root.innerHTML = summary.map((country) => `<details><summary><strong>${country.flagEmoji} ${country.country}</strong> · ${country.daysTotal} días</summary>${country.cities.map((city) => `<div class="world-rich-item"><div><strong>${city.city}</strong></div><div>${city.daysTotal} días</div></div>`).join("")}</details>`).join("");
}
