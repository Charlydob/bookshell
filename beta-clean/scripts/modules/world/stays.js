...
function renderDistribution(byCountry = [], totalDays = 0) {
  ...
  const legend = byCountry.map((country, idx) => {
    const colorClass = `world-dist-color-${(idx % 8) + 1}`;
    return `<li class="world-dist-legend-item"><span class="world-dot ${colorClass}"></span><span class="world-dist-label">${flagFromCountryCode(country.countryCode)} ${esc(country.country)}</span><span class="world-dist-pct">${pct(country.days, totalDays).toFixed(0)}%</span></li>`;
  }).join("");
...
function render() {
  ...
  const countries = stats.byCountry.map((country) => {
    ...
    const cities = country.cities.map((city) => {
      const rows = city.stays.map((stay) => `<li class="world-stays-entry"><span class="world-stays-entry-main">${esc(stay.city || "Sin ciudad")} · ${Number(stay.daysTotal || 0)} día${Number(stay.daysTotal || 0) === 1 ? "" : "s"}</span><small class="world-stays-entry-dates">${stay.startDate && stay.endDate ? `${fmtDate(stay.startDate)} → ${fmtDate(stay.endDate)}` : "Sin fechas exactas"}</small><div class="world-stays-entry-actions"><button type="button" data-world-stay-edit="${esc(stay.id)}">Editar</button><button type="button" data-world-stay-delete="${esc(stay.id)}">Eliminar</button></div></li>`).join("");
...
function openModal(stay = null) {
  ...
  modal.innerHTML = `<div class="world-stay-modal__backdrop" data-world-stay-close></div><section class="world-sheet world-stay-modal__sheet"><div class="world-sheet-header"><h3>${stay ? "Editar estancia" : "Añadir estancia"}</h3><button type="button" class="world-stay-close-x" data-world-stay-close aria-label="Cerrar">✕</button></div><div class="world-stay-block"><label class="world-stay-field"><span>Lugar</span><input data-world-stay-search placeholder="Buscar ciudad, país o dirección"></label><button type="button" class="world-optional-location-btn" data-world-use-current>Usar ubicación actual</button><div class="world-stay-search-results" data-world-stay-search-results hidden></div></div><div class="world-edit-grid"><label class="world-stay-field"><span>Ciudad</span><input data-world-stay-city placeholder="Ciudad"></label><label class="world-stay-field"><span>Región / provincia</span><input data-world-stay-region placeholder="Región/provincia"></label><label class="world-stay-field"><span>País</span><input data-world-stay-country placeholder="País"></label><label class="world-stay-field"><span>Bandera / código país</span><input data-world-stay-flag placeholder="🇵🇪 PE"></label><input data-world-stay-country-code hidden><label class="world-stay-field"><span>Fecha inicio</span><input type="date" data-world-stay-start></label><label class="world-stay-field"><span>Fecha fin</span><input type="date" data-world-stay-end></label><label class="world-stay-field world-stay-manual-days"><span>Días totales</span><input inputmode="numeric" data-world-stay-total-days placeholder="Días totales"><small>Úsalo solo si no recuerdas fechas exactas</small></label></div><button type="button" class="world-save" data-world-stay-save>Guardar</button></section>`;
