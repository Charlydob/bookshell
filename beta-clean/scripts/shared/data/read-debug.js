...
const METRICS_STORAGE_KEY = "bookshell:data-metrics:v1"
...
function looksHeavyDataPath(path = "") {
  const safePath = String(path || "").trim()
...
function getPathRiskReason(path = "", { bounded = false } = {}) {
  ...
  if (looksHeavyDataPath(safePath)) return "heavy-branch-read"
  return ""
}
...
export function logFirebaseBytesRisk({ path = "", reason = "", viewId = "global", module = "", estimatedCount = null } = {}) {
  ...
  console.warn(`[data:risk] module=${resolvedModule} path=${path} reason=${reason || "unspecified"}`, item)
...
export function logFirebaseWrite({
  ...
  console.info(`[data:write] module=${resolvedModule} path=${path} reason=${reason || "write"}`)
}
...
export function logFirebaseRead({
  ...
  if (mode === "get") {
    console.debug(`[data:get] module=${resolvedModule} path=${path} reason=${reason || "get"}`, item)
    return
  }
  console.debug(`[data:listener] requested module=${resolvedModule} path=${path} mode=${mode} reason=${reason || "listen"}`, item)
}
...
function detachListenerId(id, { invoke = true, stopReason = "" } = {}) {
  ...
  console.debug(`[data:listener] stop module=${entry.module || "unknown"} path=${entry.path || ""} mode=${entry.mode || "onValue"} reason=${stopReason || entry.reason || ""}`, {
    at: nowIso(),
    viewId: entry.viewId,
    path: entry.path || "",
    key: entry.key || "",
...
export function registerViewListener(viewId, unsubscribe, meta = {}) {
  ...
  if (existingId) {
    ...
    if (samePath && sameMode) {
      ...
      console.warn(`[data:listener] skip-duplicate module=${resolvedModule} path=${String(meta.path || "").trim()} mode=${String(meta.mode || "onValue").trim()} reason=${String(meta.reason || "").trim() || "listen"}`)
      return () => {
  ...
  if (meta.path) {
    if (looksHeavyDataPath(meta.path) && !meta.bounded) {
    ...
    console.debug(`[data:listener] start module=${resolvedModule} path=${meta.path} mode=${meta.mode || "onValue"} reason=${meta.reason || ""}`, {
      at: nowIso(),
      viewId: safeViewId,
      path: meta.path,
      key: fallbackKey,
...
export function startListenerOnce(key, targetRef, handler, options = {}, onValueImpl = null) {
  ...
  if (existingId) {
    console.debug('[data:listener:skip-duplicate]', { scope: viewId, key: safeKey, path })
    return () => stopListener(safeKey, viewId)
  }
  console.debug('[data:listener:start]', { scope: viewId, key: safeKey, path })
...
export function stopListener(key, scope = 'global') {
  ...
  if (!id) return
  const entry = state.activeListeners.get(id)
  console.debug('[data:listener:stop]', { scope: safeScope, key: safeKey, path: String(entry?.path || '') })
  detachListenerId(id, { invoke: true, stopReason: 'manual' })
}
...
export function clearDataMetrics() {
...
export const clearFirebaseMetrics = clearDataMetrics

export function exposeFirebaseReadDebug() {
  if (typeof window === "undefined") return
  window.__bookshellDebug = window.__bookshellDebug || {}
  window.__bookshellDebug.dataReads = () => ({
    reads: [...state.reads],
    counts: Object.fromEntries(state.counts.entries()),
    activeListeners: [...state.activeListeners.values()],
    bytesRisk: [...state.bytesRisk],
    suspicious: state.reads.filter((r) => looksHeavyDataPath(r.path) && !r.bounded),
  })
  window.__bookshellDebug.dataMetrics = (rangeKey = "since-start") => getFirebaseMetricsSnapshot(rangeKey)
  window.__bookshellDebug.clearDataMetrics = () => clearDataMetrics()
