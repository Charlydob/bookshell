...
import {
  ...
  subscribeToBooks,
  subscribeToGenres,
  subscribeToReadingLog,
  updateBookFavorite,
  updateBookProgress,
  uploadBookPdf
} from "../data/booksRepository.js";
...
import { renderFinishedCharts } from "../charts/donutChart.js";

const getBooks = () => appState.books;
const getReadingLog = () => appState.readingLog;
const getFilterState = () => appState.filterState;
const getCalendarState = () => appState.calendar;
const getGenres = () => appState.genreOptions;
const getDonutState = () => appState.donut;
const getBookDetailId = () => appState.bookDetailId;

let books = getBooks();
let readingLog = getReadingLog();
let genreOptions = getGenres();
let currentCalYear = getCalendarState().year;
let currentCalMonth = getCalendarState().month; // 0-11
let calViewMode = getCalendarState().viewMode || "month";
const filterState = getFilterState();
const donutState = getDonutState();
...
subscribeToGenres((val) => {
  const arr = parseGenresValue(val);
  genreOptions = arr ? sortLabels(arr) : [...DEFAULT_GENRES];
  setGenreOptions(genreOptions);
  populateGenreSelect();
});
...
$bookForm.addEventListener("submit", async (e) => {
  ...
  if (file) {
    try {
      const { url, storageId } = await uploadBookPdf(id, file);
      bookData.pdfUrl = url;
      if (!id) bookData._fixedStorageId = storageId;
    } catch (err) {
  ...
  try {
    await saveBookRecord(id, bookData);
  } catch (err) {
...
function applyDonutFilter(selection, type = "search") {
  ...
  const previousType = donutState.activeType;
  if (selection && !isAggregated && previousType && previousType !== type) {
    applyDonutFilter(null, previousType);
  }

  if (!selection || isAggregated) {
    if (donutState.activeType === "genre") {
      updateFilterState({
        genres: donutState.backupGenres ? new Set(donutState.backupGenres) : new Set()
      });
      filterState.genres = getFilterState().genres;
      renderFilterChips();
    }
    if (donutState.activeType === "search") {
      const restored = donutState.backupQuery ?? filterState.query ?? "";
      filterState.query = restored;
      if ($booksFilterSearch) {
        $booksFilterSearch.value = restored;
      }
    }
    updateDonutState({ activeType: null, backupQuery: null, backupGenres: null });
    renderBooks();
    return;
  }
  ...
  if (isGenreFilter) {
    if (donutState.activeType !== "genre") {
      updateDonutState({ backupGenres: new Set(filterState.genres) });
    }
    updateDonutState({ activeType: "genre" });
    updateFilterState({ genres: label ? new Set([label.toLowerCase()]) : new Set() });
    filterState.genres = getFilterState().genres;
    renderFilterChips();
  } else {
    if (donutState.activeType !== "search") {
      updateDonutState({ backupQuery: filterState.query || "" });
    }
    updateDonutState({ activeType: "search" });
...
function renderBookCharts(finishedIds) {
  renderFinishedCharts({
    finishedIds,
    books,
    section: $booksChartsSection,
    chartGenre: $chartGenre,
    chartAuthor: $chartAuthor,
    chartCentury: $chartCentury,
    onGenreSelect: (selection) => applyDonutFilter(selection, "genre"),
    onSearchSelect: (selection) => applyDonutFilter(selection, "search")
  });
}
...
function buildReadingSpine(id) {
  ...
  const commit = () => {
    ...
    input.value = "";
    safeUpdateBookProgress(id, safe);
  };
...
if (inlineInput) {
  ...
  inlineInput.addEventListener("blur", commit);
    ...
    const handleMarkFinished = () => safeMarkBookFinished(id);
    ...
    if (finished) {
      ...
      btnDone.addEventListener("click", handleMarkFinished);
    }
...
// Render terminados (plegable)
  ...
  renderBookCharts(finishedIds);
}

// === Actualizar progreso y log de lectura ===
async function safeUpdateBookProgress(bookId, newPage) {
  try {
...
async function safeMarkBookFinished(bookId) {
  const book = books[bookId];
  if (!book) return;
  try {
    await markBookFinished(bookId, book);
  } catch (err) {
