...
function renderTabs() {
  document.querySelectorAll(".videosHub__tab").forEach((btn) => {
    btn.classList.toggle("is-active", btn.dataset.hubTab === state.viewTab);
  });
  document.querySelectorAll(".videosHub__panel").forEach((panel) => {
...
function bindEvents() {
  ...
  els.createBtn?.addEventListener("click", () => { void createVideo(); });
...
function subscribeData() {
  ...
  const stop = onValue(ref(db, `${state.path}/videos`), (snap) => {
    ...
    if (!hasSelected) {
      const first = Object.entries(state.videos).sort((a, b) => (Number(b[1]?.updatedAt) || 0) - (Number(a[1]?.updatedAt) || 0))[0];
...
function cacheElements() {
  els.root = document.getElementById("view-videos-hub");
  if (!els.root) return;
  els.createBtn = document.getElementById("videos-hub-create-btn");
  els.list = document.getElementById("videos-hub-list");
  els.search = document.getElementById("videos-hub-search");
  els.filterStatus = document.getElementById("videos-hub-filter-status");
  ...
  els.metrics = document.getElementById("videos-hub-metrics");

  els.statsRange = document.getElementById("videos-hub-stats-range");
  els.statsStatus = document.getElementById("videos-hub-stats-status");
  els.statsCategory = document.getElementById("videos-hub-stats-category");
  els.kpis = document.getElementById("videos-hub-kpis");
  els.chartLine = document.getElementById("videos-hub-chart-line");
  els.chartBars = document.getElementById("videos-hub-chart-bars");
  els.chartDonut = document.getElementById("videos-hub-chart-donut");
  els.videoCompare = document.getElementById("videos-hub-video-compare");
}
...
export async function init() {
  ...
  if (!uid) throw new Error("[videos-hub] usuario no autenticado");

  state.uid = uid;
  state.path = `v2/users/${uid}/videosHub`;
...
export async function resetToMainView() {
  state.viewTab = "list";
  renderTabs();
}
